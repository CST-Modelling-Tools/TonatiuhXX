...
- Hybrid-CPU scheduling experiment: on Qt 6.9 and newer, each long-lived ray worker temporarily requests `QThread::QualityOfService::High` and restores its previous service level on exit. This addresses observed Windows headless runs where 20 runnable workers were time-sliced across the four i7-12700K E-cores despite unrestricted affinity; external GUI/headless and cross-platform validation remains required.
...
- Validate the Qt 6.9+ ray-worker `High` quality-of-service request on hybrid Windows CPUs and confirm that it improves headless utilization without regressing GUI throughput or Linux/macOS behavior; Qt 6.8 and older retain their existing scheduling behavior.
