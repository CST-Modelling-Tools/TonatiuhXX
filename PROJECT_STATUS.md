...
- Tonatiuh++ now has one supported parallel ray-tracing implementation: the shared QCore-compatible `RayTraceExecutor`, using QtConcurrent, the global Qt thread pool, one shared mutex-protected random stream, and serialized photon-buffer access. GUI `MainWindow::Run()`, headless `trace-scene`, benchmark tracing, and `tn.traceScene()` all call this executor.
- RayTraceExecutor scheduling now uses adaptive fine-grained work items instead of 100 fixed coarse partitions. Work items are capped at 100,000 rays and the executor targets eight scheduling opportunities per available global-pool thread when the ray count permits, allowing QtConcurrent to rebalance work dynamically without restoring RayTraceRunner or adding another backend.
...
- Adaptive RayTraceExecutor work distribution requires external performance and scientific-output validation at representative 10 million, 50 million, and 500 million ray counts; scheduling boundaries changed, while the shared RNG algorithm, seed input, physics, sampling code, and benchmark formulas remain unchanged.
