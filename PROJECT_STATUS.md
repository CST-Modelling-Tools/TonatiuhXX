...
- Tonatiuh++ now has one supported parallel ray-tracing implementation: the shared QCore-compatible `RayTraceExecutor`, using long-lived workers on the global Qt thread pool, one shared mutex-protected random stream, and serialized photon-buffer access. GUI `MainWindow::Run()`, headless `trace-scene`, benchmark tracing, and `tn.traceScene()` all call this executor.
...
- Opt-in tracing diagnostics are aggregate-only: one bounded scheduling summary is emitted at trace start and one worker/RNG summary after completion. Intrusive worker-thread logging at every chunk start and finish was removed after it increased a Windows 10-million-ray benchmark from roughly 15.5 seconds to 63.9 seconds; diagnostic timing should not be treated as representative if its measurement overhead materially changes the workload.
...
- GUI tracing and flux analysis prepare borrowed contexts over their persistent `SceneTreeModel`/`InstanceNode` hierarchy, existing sun instance, RNG, photon buffer, and export selections. Headless trace, benchmark, and `tn.traceScene()` prepare owned contexts containing exactly one newly built instance tree plus the sun instance and seeded RNG, retained through executor completion.
...
- The RayTraceExecutor worker-loop strategy requires external performance and scientific-output validation at representative 10 million, 50 million, and 500 million ray counts; chunk boundaries and work acquisition changed, while the shared RNG algorithm, seed input, physics, sampling code, and benchmark formulas remain unchanged.
